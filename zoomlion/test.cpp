...
//template<class T>
double zk_algorithm::test_hoist(cv::String imgs_path, cv::String test_y) {
	...
	cv::String file_txt = "F:\\c++\\SVM_ZOOM\\SVM_ZOOM\\train_data.txt";
	cv::String label_txt = "F:\\c++\\SVM_ZOOM\\SVM_ZOOM\\train_label.txt";
	...
	while (y_stream >> y_string) {
		...
		*/
		...
		int result = my_algorithm_pos_test(imgs_file[i]);
		if (result == y_data) {
			TP += 1;
		}
