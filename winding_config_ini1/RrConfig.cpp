...
namespace rr
{
	...
	std::string RrConfig::ReadString(const char* section, const char* item, const char* default_value)
	{
		...
		//str.erase(0);
		//str.pop_back();
		//std::cout << "str.begain = " << str << std::endl;
	...
	int RrConfig::ReadInt(const char* section, const char* item, const int& default_value)
	{
		...
		//int aa = k_v.at[1];
		//for (it_item = k_v.begin(); it_item != k_v.end(); it_item++) {
			//std::cout << it_item->first << " " << it_item->second << std::endl;
		//	std::cout << "it_item->first = " << it_item->first << std::endl;
		//	std::cout << "it_item->second " << it_item->second << std::endl;
		//}
